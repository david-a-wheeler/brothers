...
**Get Tiled version 1.12.2 or newer.** That is the version we've tested with.
Older ones may not understand everything in the level files. You can check which
version you have with **Help → About Tiled**.
...
- **macOS**: download the `.dmg`, open it, and drag **Tiled** into your
  ...
  unidentified developer. If so, right-click the app icon and choose **Open**,
  then **Open** again in the dialog. (You only have to do this once.)
- **Linux**: you have two choices.

  Your distribution probably packages Tiled, which is the easiest route:

  ```bash
  sudo apt-get install tiled     # Debian, Ubuntu, Mint
  sudo dnf install tiled         # Fedora
  ```

  Check what you got with `tiled --version`. If it's older than 1.12.2 (some
  distributions ship an old one), use the AppImage from the download page
  instead; it is always current and doesn't need installing:
...
- **macOS**: open **Terminal** (press ⌘+Space, type `Terminal`, press Enter)
  ...
  the developer tools (accept), or you can get Python from
...
2. Click one of the long thin rectangles. Its Class is `Wall`. Its position and
   size *are* the wall, and there's nothing else to set.
...
| `massMult` | How heavy he is. Independent of size, so a small heavy brother is allowed | David `1.2`, Ken `1.0` |
...
| `inViscosity` | *Extra* drag that applies only while he's inside it, making a deep bog. `0` = none | `0` |
...
| Property | What it does | Default |
| --- | --- | --- |
| `name` | The level's title, shown in the game | (none) |
| `intro` | A sentence or two shown once, the first time the level is played. Use it to teach whatever the level is about | (none) |
...
pack folder under `packs/`. Then reload the browser page, because a level is only
read from disk once per page load.
...
The game caught a bug. Click **Copy problem report** and send it to David; that
report says exactly what happened. Your level file is fine; it's the game
that got confused.
